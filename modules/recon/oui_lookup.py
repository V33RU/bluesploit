...
# Path to the bundled IEEE OUI snapshot. Generated by `scripts/fetch_oui.py`
...
# Process-wide cache. Lazily merges BUILTIN_OUI_DB (hand-picked short names)
# with the IEEE snapshot (~40k entries) on first lookup.
_oui_db_cache: Optional[Dict[str, Tuple[str, str]]] = None


def _load_oui_db() -> Dict[str, Tuple[str, str]]:
    """Return the OUI database: `{oui_colon_upper: (short_name, full_name)}`.

    The inline `BUILTIN_OUI_DB` is loaded first and always wins on
    conflicts because its short names are hand-picked. The compressed
    IEEE snapshot is layered on top to cover the long tail; entries
    from there derive a short name from the first word of the vendor
    string.
    """
    global _oui_db_cache
    if _oui_db_cache is not None:
        return _oui_db_cache
    db: Dict[str, Tuple[str, str]] = dict(BUILTIN_OUI_DB)
    if _OUI_CSV_GZ.is_file():
        try:
            with gzip.open(_OUI_CSV_GZ, "rt", encoding="utf-8") as f:
                first = f.readline()  # header
                if not first.lower().startswith("oui"):
                    # Old / unexpected format, fall back to builtin only.
                    _oui_db_cache = db
                    return db
                for line in f:
                    line = line.rstrip("\n")
                    oui_hex, _, vendor = line.partition(",")
                    if len(oui_hex) != 6 or not vendor:
                        continue
                    formatted = (
                        f"{oui_hex[0:2]}:{oui_hex[2:4]}:{oui_hex[4:6]}"
                    ).upper()
                    if formatted in db:
                        continue  # builtin keeps its nicer short name
                    short = vendor.split()[0] if vendor.strip() else "Unknown"
                    db[formatted] = (short, vendor)
        except OSError:
            pass
    _oui_db_cache = db
    return db
...
class Module(ScannerModule):
    ...
    def _lookup_builtin(self, oui: str) -> Optional[Tuple[str, str]]:
        """Lookup OUI in the bundled database (inline + IEEE snapshot)."""
        return _load_oui_db().get(oui.upper())
